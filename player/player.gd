...
const DAMAGE_HORIZONTAL_VELOCITY = 400
const DAMAGE_VERTICAL_VELOCITY = -200
...
func on_bounce(force) -> void:
	velocity.y = -100 * force;
	
func damaged_physics() -> void:
	jump_sound.pitch_scale = 0.4
	velocity.y = DAMAGE_VERTICAL_VELOCITY;
	var direction = Input.get_axis("move_left" + action_suffix, "move_right" + action_suffix)
	velocity.x = -1 * direction * DAMAGE_HORIZONTAL_VELOCITY;
	jump_sound.play()
